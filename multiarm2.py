...
class BanditReproduction(DefaultReproduction):
    ...
    def reproduce(self, config, species, pop_size, generation):
        ...
        if generation > 0:
            ...
            for mutant, old_fitness, mutation in self.records[-1]:
                fit_delta = mutant.fitness - old_fitness
                
                mutation_delta.append((mutation, fit_delta))
            # print(mutation_delta)
            self.bandit.update_batch(mutation_delta)
...
class BanditGenome(DefaultGenome):
    ...
    def mutate_add_node(self, config):
        ...
        conn_key = choice(list(self.connections.keys()))
        conn_to_split = self.connections[conn_key]
        ...
        # Delete this connection and create two new connections joining its nodes via
